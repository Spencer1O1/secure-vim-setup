...
function! s:LooksLikePath(text)
  ...
  return token !~# '^\a\+://' && (token =~# '[/\\]' || token =~# '^\~' || token =~# '^\.\.\?$' || token =~# '^[A-Za-z]:')
endfunction
...
function! s:PathComplete(findstart, base)
  if a:findstart
    ...
    return b:path_complete_active ? strlen(before) - strlen(s:PathToken(before)) : col('.') - 1
  endif
...
if exists('+autocomplete')
  set autocomplete infercase
  set autocompletedelay=0
  let s:complete_sources = 'F'.expand('<SID>').'PathComplete,o^15,t^10,.^10,w^5,b^5,u^5'
...
function! s:LegacyAutoComplete(timer)
  ...
  let before = strpart(getline('.'), 0, col('.') - 1)
  if before =~# '\k$' || s:LooksLikePath(before) || !empty(&l:omnifunc)
...
function! s:ScheduleLegacyComplete()
  ...
  let s:completion_timer = timer_start(0, function('<SID>LegacyAutoComplete'))
endfunction
...
function! s:AcceptCompletion()
  ...
  if exists('*complete_info') && get(complete_info(), 'selected', -1) < 0
    return "\<C-n>\<C-y>"
  endif
  return "\<C-y>"
endfunction

function! s:EscapeCompletion()
  if pumvisible()
    let b:suppress_autocomplete_once = 1
    return "\<C-e>"
  endif
